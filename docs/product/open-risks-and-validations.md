...
- **Subagent run cost.** Walking flows with subagents isn't free. Mitigated by onboarding + incremental (D8), but watch the per-flow cost. Under managed billing (D25) this is a **direct cost line** — Overstory pays for the Opus analysis — so per-flow cost must stay below what the user will pay.
- **Repo access + code egress (D25/D27).** With managed LLM (D25) and granted repo access (D27), Overstory both reads the repo (read-only GitHub App) and sends code server-side for analysis. Egress is **by-design** now, not an accident. Two things to watch: **(a) adoption gate** — granting repo access is a far bigger ask than an API key; the trust barrier D24 originally dodged is now the real buyer hurdle (dogfood is trivial; buyers are not). **(b) credential surface** — we hold GitHub App installation access; mitigated by minting short-lived (~1h) tokens on demand from a KMS-held app key, storing no long-lived per-repo secrets, and least-privilege read-only scopes (`contents:read`, `pull_requests:read`). Fallback if access blocks buyers: a local-only / BYO-key mode that keeps code on-machine (reintroduces the D24 path as an option).
